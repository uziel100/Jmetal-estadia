    ...
    for (int i = 0; i < swarmSize; i++) {
      ...
      if (this.localBest[i] != this.neighborhoodBest[i]) {
        for (int var = 0; var < particle.getNumberOfVariables(); var++) {
          double G;
          G = particle.getVariableValue(var) + c * (localBest[i].getVariableValue(var) + neighborhoodBest[i].getVariableValue(var) - 2 * particle.getVariableValue(var)) / 3.0;
      ...
      if (localBest[i] != neighborhoodBest[i]) {
        for (int var = 0; var < particle.getNumberOfVariables(); var++) {
          speed[i][var] = weight * speed[i][var] + r1 * (localBest[i].getVariableValue(var) - particle.getVariableValue(var)) +
    ...
    for (int i = 0; i < swarmSize; i++) {
      ...
      for (int var = 0; var < particle.getNumberOfVariables(); var++) {
        ...
        if (particle.getVariableValue(var) < problem.getLowerBound(var)) {  
          ...
          newPosition = particle.getLowerBound(var) + rnd * (particle.getUpperBound(var) - particle.getLowerBound(var));          
          particle.setVariableValue(var, newPosition);
          //Random Back
          speed[i][var] =  -randomGenerator.nextDouble() * speed[i][var];
        }
