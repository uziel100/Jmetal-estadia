    ...
    protected void initializeUniformWeight() {
        if ((problem.getNumberOfObjectives() == 2) && (populationSize <= 300)) {
            ...
            try {
                ...
                //String path = Paths.get(this.getClass().getResource(dataFileName).toURI()).toString();                             
                String path = Paths.get(VectorFileUtils.class.getClassLoader().getResource(dataFileName).toURI()).toString();                             
                InputStream in = getClass().getClassLoader().getResourceAsStream(path + "/" + dataFileName);
